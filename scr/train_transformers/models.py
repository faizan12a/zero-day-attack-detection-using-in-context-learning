...
class TF(torch.nn.Module):
    ...
    # def forward(self, x_features,x_labels):
    #     x = torch.empty(x_features.shape[0], 2 * self.nos, self.input_dim)
    #     samples, nos, wc = x_labels.shape  # Get the current shape
    #     padding_size = self.input_dim  - wc
    #     padding = -1 * torch.ones((samples, nos, padding_size), dtype=x_labels.dtype, device=x_labels.device)
    #     x_labels_padded = torch.cat((x_labels, padding), dim=2)
    #     x=x.to(self.device)
    #     x[:, 0::2, :] = x_features
